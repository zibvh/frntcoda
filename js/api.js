...
export async function getDocs(ref){const name=ref.name||ref.collection;const constraints=ref.constraints||[];const params=new URLSearchParams();for(const c of constraints){if(c.op==='orderBy')params.set('sort',(c.value==='desc'?'-':'')+c.field);else if(c.op==='limit')params.set('limit',c.value);else if(c.op==='in'||c.op==='array-contains-any'){params.set(c.field,JSON.stringify(c.value));params.set(c.field+'__op','in');}else { params.set(c.field, typeof c.value==='object'?JSON.stringify(c.value):String(c.value)); if(c.op && c.op!=='==') params.set(c.field+'__op',c.op); }}const data=await request(`/${name}?${params}`);return {docs:data.map(d=>({id:d.id,exists:()=>true,data:()=>({...d})})),empty:data.length===0,size:data.length,forEach(fn){this.docs.forEach(fn)}};}
